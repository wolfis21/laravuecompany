    ...
    alerta.fire({
        ...
        if(result.isConfirmed) {
            form.delete(route('employees.destroy', id),{
                onSuccess: () => {ok('Employees deleted successfully')} 
            });
        }
    });
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-12">
                ...
                <div class="table-auto border border-gray-400">
                    ...
                    <tbody>
                        <tr v-for="emp, i in employees.data" :key="emp.id">
                            ...
                            <th class="border border-gray-400 px-2 py-2">{{ emp.name }}</th>
                            <th class="border border-gray-400 px-2 py-2">{{ emp.email }}</th>
                            <th class="border border-gray-400 px-2 py-2">{{ emp.phone }}</th>
                            <th class="border border-gray-400 px-2 py-2">{{ emp.department }}</th>
                            ...
                            <th class="border border-gray-400 px-2 py-2">
                                <DangerButton @click="$event => deleteEmployees(emp.id, emp.name)">
            ...
            <Modal :show="modal" @close="closeModal">
                <h2 class="p-3 text-lg-center font.medium text-hray-900">{{ title }}</h2>
                <div class="p-3">
                ...
                <div class="p-3">
                    ...
                    <TextInput id="phone" v-model="form.phone" type="text" class="mt-1 block w-3/4" placeholder="Phone"></TextInput>
                    <InputError :message="form.errors.phone" class="mt-2"></InputError>
                </div>
                <div class="p-3">
                    <InputLabel for="department_id" value="Department:"></InputLabel>
                    <SelectInput id="department_id" :options="departments" v-model="form.department_id" type="text" class="mt-1 block w-3/4"></SelectInput>
